    ...
    const handleMakeFilm = async () => {
        ...
        const processSceneWithRetry = async (scene: Scene) => {
            ...
            for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                try {
                    const videoResult = await generateVideo(imageToUse, scene, styleState.aspectRatio, (scene.useAssets !== false) ? chunk.assets : []);
                    onSceneUpdate(chunk.id, scene.id, { videoUrl: videoResult.url, videoAssetId: videoResult.assetId });
                    return;
                } catch (e: any) {
